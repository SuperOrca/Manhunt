...
public class ManhuntCommand {
    ...
    public void speedrunners(final CommandSender sender, final @Default("list") String mode, final @Named("player") EntitySelector<Player> targets) {
        if ("list".equals(mode)) {
    ...
    @Subcommand("hunters")
    @AutoComplete("add|remove|list *")
    public void hunters(final CommandSender sender, final @Default("list") String mode, final @Named("player") EntitySelector<Player> targets) {
        if ("list".equals(mode)) {
